...
import { Scissors, Download, RefreshCw, Music, Link as LinkIcon, CheckCircle, ExternalLink, Clock, Wand2, Timer, Settings2, FileAudio, Layers, ChevronRight } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleFileSelect = async (selectedFile: File) => {
    ...
    try {
      ...
      if (mode === 'silence') {
        ...
        // This function now runs async due to OfflineAudioContext rendering
        bufferList = await removeSilence(audioBuffer);
        
        // If user wants a single merged file
      ...
      // 3. Convert to Blobs (WAV or MP3)
      setProcessingStage(`Convertendo para ${outputFormat.toUpperCase()}...`);
      
      // Use setTimeout to allow UI to update before heavy encoding
      setTimeout(async () => {
        try {
          ...
          // 4. Handle Download generation (ZIP or Single File)
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center py-8 sm:py-12 px-4 sm:px-6 lg:px-8">
      {/* Container aumentado para max-w-6xl para melhor distribuição em PC */}
      <div className="w-full max-w-6xl space-y-8 sm:space-y-10">
        ...
        <div className="text-center space-y-4 sm:space-y-5">
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button 
              onClick={handleReset}
              className="bg-indigo-600 p-3 rounded-xl shadow-lg shadow-indigo-200 cursor-pointer hover:bg-indigo-700 hover:scale-105 transition-all duration-200 group"
              title="Voltar ao início"
            >
              <Scissors className="w-6 h-6 sm:w-8 sm:h-8 text-white group-hover:rotate-12 transition-transform" />
            </button>
            <h1 className="text-2xl sm:text-4xl lg:text-5xl font-extrabold text-slate-900 tracking-tight text-center leading-tight">
              Sebas Audio Cutter <span className="text-indigo-600">Pro</span>
            </h1>
          </div>
          
          <div className="text-slate-600 max-w-3xl mx-auto space-y-3">
            <h2 className="text-base sm:text-xl font-bold text-slate-800 px-2">
              Liberte a sua produtividade: O corte de áudio nunca mais será o mesmo!
            </h2>
            <p className="text-xs sm:text-base leading-relaxed hidden sm:block">
              Apresentamos o melhor e mais prático cortador de áudio automático do mercado. Elimine o trabalho manual e ganhe tempo na sua produção.
            </p>
            <p className="text-xs sm:hidden leading-relaxed">
              O cortador automático mais prático do mercado. Otimize seu fluxo com IA.
            </p>
          </div>
        </div>
          ...
          <div className="p-4 sm:p-8 lg:p-10 space-y-6">
            ...
            {!file || (segments.length === 0 && isProcessing) ? (
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
                 ...
                 {/* Left Column (Desktop): Mode Selection */}
                 ...
                 {/* Right Column (Desktop): Settings & Dropzone */}
                 <div className="lg:col-span-8 flex flex-col gap-6">
                    
                    {/* Settings Panel */}
                    <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100 space-y-4 animate-in fade-in duration-300">
                        ...
                        {mode === 'silence' && (
                            <p className="text-xs text-slate-500 mt-2 italic flex items-center gap-1.5 bg-indigo-50/50 p-2 rounded-lg border border-indigo-100/50">
                                <Wand2 className="w-3.5 h-3.5 text-indigo-500" /> 
                                O filtro inteligente isolará vocais (300Hz-3.5kHz) e removerá trechos vazios.
                            </p>
                        )}
                    </div>
